...
def _looks_like_jwt(token: str) -> bool:
    """Three dot-separated base64 segments with a decodable JSON payload."""
    if not token:
        return False
    parts = token.split(".")
    if len(parts) != 3:
...
def _admin_key_match(candidate: str) -> bool:
    ...
    if not expected or not candidate:
        return False
    # Constant-ish comparison is fine here; these are short opaque strings, not secrets in logs.
...
def verify_admin_key(request: Request) -> bool:
    ...
    if auth.startswith("Bearer "):
        token = auth[7:]
        # Only treat Authorization as an admin key if it does NOT look like a JWT.
...
def authenticate_jwt(request: Request) -> Optional[Identity]:
    ...
    Returns None if Authorization carries an admin key (not a JWT) — callers
    that want admin-key access should use verify_admin_key + authenticate_internal.
    """
    auth = request.headers.get("authorization") or ""
